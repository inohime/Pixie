...
namespace pxe {
	PixieRenderer::PixieRenderer(SDL_Window *window, UINT width, UINT height) : surfaceWidth(width), surfaceHeight(height),
		factory(nullptr), device(nullptr), cmdQueue(nullptr), cmdAlloc(nullptr), rootSig(nullptr), 
		pipelineState(nullptr), cmdList(nullptr),
		...
		rtvHeap(nullptr), srvHeap(nullptr), swapchain(nullptr), vertexBuffer(nullptr),
		fence(nullptr) {
	...
	void PixieRenderer::createPipelineState() {
		...
		{
			// Define the geometry for a triangle (remove later)
		...
		// Create the texture.
		{
			...
			textureDesc.Width = textureWidth; // change
